    ...
    const tokensToVerify: TokenToVerify[] = [
      {
        ...
        svelteExpectedColumn: 9, // EXPECTED CORRECT: TSX 'foo1' should map to Svelte 'foo1' (col 9 of '\tfunction foo1()')
        tsxExactMatch: 'function foo1()'
      }
    ];

    for (const tokenTest of tokensToVerify) {
      const { svelteToken, svelteExpectedLine, svelteExpectedColumn, tsxTargetHint, tsxExactMatch, occurrenceInTsx = 1 } = tokenTest;
      let foundTsxLine = -1;
      let foundTsxCol = -1;
      let matchCount = 0;
      ...
      if (!tokenTest.isRegex) { // Standard content check
        assert.strictEqual(mappedSvelteToken, svelteToken, `Mapped Svelte content for "${svelteToken}" does not match. Expected "${svelteToken}", got "${mappedSvelteToken}" from Svelte line: "${svelteLineContent}"`);
      }
