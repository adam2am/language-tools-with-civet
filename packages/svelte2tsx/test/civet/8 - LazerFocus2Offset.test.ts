...
describe('8 - LazerFocus2Offset: template mapping offset analysis #current', () => {
  // Each case tests mapping for a specific invocation token and expected offset
  ...
  for (const { name, token, expectOffset } of fixtures) {
    it(`${name}: template usage '${token}' mapping offset should be ${expectOffset}`, () => {
      ...
      const idx = tsxCode.indexOf(token, firstIdx + token.length);
      ...
      const usageLine0 = originalLines.findIndex(line => line.includes(`onclick={${token}}`));
